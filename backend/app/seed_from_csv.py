# -*- coding: utf-8 -*-
# (tornado_env) qssss@Qsssss:~/haozong/hunlian/master/backend$ 
# python -m app.seed_from_csv
import os
import csv
import json
...
DATA_ROOTS = [
    ...
    os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "data", "advantage"))
]
...
def enum_or_none(val, allowed: set[str]):
    """
    把 CSV 里读到的枚举值做清洗：
    - None / '' / 'None'  => None
    - 不在 allowed 里的值 => None
...
def _exists_pk(table_name: str, pk_name: str, pk_val):
    if pk_val is None:
        return False
    row = db.execute(
        text(f"SELECT 1 FROM {table_name} WHERE {pk_name} = :v LIMIT 1"),
        {"v": pk_val}
    ).fetchone()
    return row is not None

def import_userto_intention(root):
    rows = to_read_csv(to_find(root, "user_intention.csv"))
    cnt = 0

    REL_GOAL_ALLOWED = {"dating", "marriage"}
    MT_ALLOWED = {"1y", "2y", "flexible", "unknown"}         # 可为 None
    CP_ALLOWED = {"want", "dont_want", "flexible", "unknown"}# 可为 None
    FV_ALLOWED = {"independent", "with_parents", "flexible"} # 可为 None

    for r in rows:
        rid = to_int(r.get("id"))
        if _exists_pk("user_intention", "id", rid):
            continue
        ...
        db.add(UserIntention(
            ...
            # relationship_goal 在模型里 nullable=False，因此空值用默认 'dating'
            relationship_goal = (r.get("relationship_goal") if str(r.get("relationship_goal")).strip() in REL_GOAL_ALLOWED else "dating"),
            ...
            # 关键：这三个字段允许为 NULL，空白/非法 => None（数据库 NULL）
            marriage_timeline = enum_or_none(r.get("marriage_timeline"), MT_ALLOWED),
            child_plan        = enum_or_none(r.get("child_plan"), CP_ALLOWED),
            family_view       = enum_or_none(r.get("family_view"), FV_ALLOWED),
...
def import_advantage(root):
    rows = to_read_csv(to_find(root, "advantage.csv"))
    cnt = 0
    for r in rows:
        if db.get(Advantage, int(r["id"])): continue
        db.add(Advantage(
            id=int(r["id"]),
            title=r["title"],
            desc=r["desc"],
