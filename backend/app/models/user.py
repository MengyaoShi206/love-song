...
class ChatMessage(Base):
    ...
    from_user_id = Column(Integer, ForeignKey("user_account.id"), nullable=False, index=True)
    to_user_id = Column(Integer, ForeignKey("user_account.id"), nullable=False, index=True)

    # 会话 key，比如 "3-7" 表示用户 3 和 7 之间的会话，方便按会话拉历史
    session_key = Column(String(64), nullable=False, index=True)

    # 消息类型，后面想扩展图片、系统通知都可以用这个
    msg_type = Column(
        Enum(
            "text",      # 纯文字
            "image",     # 图片
