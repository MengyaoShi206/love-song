          ...
          <template v-else>
            ...
            >
              <div class="form-grid">
                ...
                <el-form-item label="年龄范围" prop="preferred_age_min">
...
const onSelect = (key) => { if (key === 'display') router.push('/main'); else if (key === 'match') router.push('/match') }
...
function removeQna(index) { editableQna.value.splice(index, 1) }
function addQnaFromTemplate() {
  if (!selectedTemplate.value) return ElMessage.warning('请选择一个问题')
  if (editableQna.value.some(q => q.question === selectedTemplate.value)) return ElMessage.warning('该问题已存在')
...
function cancel() { editMode.value = false }
...
//     // 规范化：择偶意向 + 生活方式
//     normalizeIntentionForSave(payload)
//     normalizeLifestyleForSave(payload)
...
function formatElFormError(err) {
  ...
  try {
    ...
    // 也可以把多条拼起来：
    // const all = fields.flatMap(f => (err[f]||[]).map((e:any)=>e.message)).filter(Boolean)
    // return all.join('；')
    return firstMsg
  } catch {
    return '表单校验未通过'
...
async function save() {
  try {
    ...
    // 1) 先跑三段表单校验（如果失败会直接 throw Error，进入 catch）
    await validateAll()

    // 2) 组装 & 规范化（保持你喜欢的“嵌套 payload”）
    const raw = {
      user_account:        display.user_account,
      user_profile_public: display.user_profile_public,
      user_intention:      display.user_intention,
...
const accountRules = {
  ...
  phone: [
    ...
    { validator: (_r, v, cb) => { if (!v) return cb(); /^[0-9+\-\s]{6,20}$/.test(String(v)) ? cb() : cb(new Error('手机号格式不正确')) }, trigger: 'blur' }
  ],
  email: [{ type: 'email', message: '邮箱格式不正确', trigger: 'blur' }],
  height_cm: [{ validator: (_r, v, cb) => { if (v==null || v==='') return cb(); const n=Number(v); if (Number.isNaN(n)) return cb(new Error('请输入数字')); if (n<120||n>230) return cb(new Error('身高范围 120~230 cm')); cb() }, trigger: 'change' }],
  weight_kg: [{ validator: (_r, v, cb) => { if (v==null || v==='') return cb(); const n=Number(v); if (Number.isNaN(n)) return cb(new Error('请输入数字')); if (n<35||n>150) return cb(new Error('体重范围 35~150 kg')); cb() }, trigger: 'change' }],
  avatar_url: [{ validator: (_r, v, cb) => { if (!v) return cb(); try { const u=new URL(String(v)); if (!/^https?:$/.test(u.protocol)) throw new Error(); cb() } catch { cb(new Error('请输入以 http/https 开头的URL')) } }, trigger: 'blur' }]
}
function maskPhone(p){ if(!p) return '—'; const s=String(p).replace(/\s+/g,''); if(s.length<7) return s; return s.slice(0,3)+'****'+s.slice(-4) }
function maskEmail(e){ if(!e) return '—'; const s=String(e); const [n,h]=s.split('@'); if(!h) return s; const m=n.length<=2? n[0]+'*' : n[0]+'***'+n.slice(-1); return `${m}@${h}` }
function calcAge(birth){ if(!birth) return null; try{ const d=new Date(birth); if(isNaN(d.getTime())) return null; const t=new Date(); let age=t.getFullYear()-d.getFullYear(); const md=(t.getMonth()+1)*100+t.getDate(); const bd=(d.getMonth()+1)*100+d.getDate(); if(md<bd) age-=1; return age>=0?age:null }catch{ return null } }
function yn(v){ if(v===true||v===1||v==='1'||v==='true') return '是'; if(v===false||v===0||v==='0'||v==='false') return '否'; return '—' }
function numOrDash(v){ return (v===0||v) ? v : '—' }
function displayCities(v){ if(!v) return '—'; try{ const arr=typeof v==='string'? JSON.parse(v):v; if(Array.isArray(arr)) return arr.join('、') }catch{} return String(v) }
...
watch(
  ...
  (v) => {
    try {
      if (Array.isArray(v)) citiesInput.value = v.join(', ')
      else if (typeof v === 'string') {
        const maybe = JSON.parse(v)
        citiesInput.value = Array.isArray(maybe) ? maybe.join(', ') : v
      } else citiesInput.value = ''
    } catch { citiesInput.value = String(v || '') }
  },
  { immediate: true }
)
const intentionRules = {
  relationship_goal: [{ required: true, message: '请选择感情目标', trigger: 'change' }],
  age_range: [{ validator: (_r,_v,cb)=>{ const a=Number(display.user_intention?.preferred_age_min); const b=Number(display.user_intention?.preferred_age_max); if(!Number.isFinite(a)||!Number.isFinite(b)) return cb(new Error('请填写年龄上下限')); if(a<18||b<18) return cb(new Error('年龄不得小于18')); if(a>b) return cb(new Error('年龄下限不得大于上限')); cb() }, trigger:'change' }],
  height_range: [{ validator: (_r,_v,cb)=>{ const a=display.user_intention?.preferred_height_min; const b=display.user_intention?.preferred_height_max; if(a==null&&b==null) return cb(); const na=Number(a), nb=Number(b); if(!Number.isFinite(na)||!Number.isFinite(nb)) return cb(new Error('身高需为数字')); if(na<140||na>220||nb<140||nb>220) return cb(new Error('身高范围 140~220cm')); if(na>nb) return cb(new Error('身高下限不得大于上限')); cb() }, trigger:'change' }],
  preferred_cities: [{ validator: (_r,_v,cb)=>{ if(!citiesInput.value) return cb(); const arr=citiesInput.value.split(/[，,]/).map(s=>s.trim()).filter(Boolean); if(!arr.length) return cb(new Error('请正确填写城市或留空')); cb() }, trigger:'blur' }]
}
function goalText(v){ const map={ dating:'恋爱', marriage:'结婚', unsure:'未定/开放' }; return map[String(v)] || v || '—' }
function timelineText(v){ const map={ '1y':'1年内', '2y':'2年内', flexible:'随时', unknown:'暂不考虑' }; return map[String(v)] || v || '—' }
function childPlanText(v){ const map={ want:'尽快', flexible:'合适再说', dont_want:'不考虑', unknown:'暂不考虑' }; return map[String(v)] || v || '—' }
function familyViewText(v){ const map={ independent:'自己住', with_parents:'和父母住' }; return map[String(v)] || v || '—' }
...
function verifyStatusText(s){ const map={0:'待审',1:'通过',2:'拒绝',3:'复审中'}; if(s==null||s==='') return '—'; const k=Number(s); return (k in map)? map[k] : String(s) }
function riskActionText(a){ const map={0:'无',1:'限流',2:'限聊',3:'封禁'}; if(a==null||a==='') return '—'; const k=Number(a); return (k in map)? map[k] : String(a) }
function fmtTs(s){ if(!s) return '—'; try{ const t=String(s); return t.length>19? t.slice(0,19).replace('T',' ') : t.replace('T',' ') }catch{ return String(s) } }

/** 生活方式：规则/列表/watch/映射/规范化 */
const lifestyleFormRef = ref()
const lifestyleRules = {
  schedule:[{required:false}], drinking:[{required:false}], smoking:[{required:false}], workout_freq:[{required:false}],
  diet:[{required:false}], pet_view:[{required:false}], spending_view:[{required:false}], saving_view:[{required:false}],
  travel_pref:[{required:false}], interests:[{required:false}], personality:[{required:false}]
}
...
watch(
  ...
  (v) => {
    try {
      ...
      travelList.value = Array.isArray(src) ? src : (typeof src === 'string' && JSON.parse(src)) || []
    } catch { travelList.value = [] }
    try {
      const src = v?.interests
      interestList.value = Array.isArray(src) ? src : (typeof src === 'string' && JSON.parse(src)) || []
    } catch { interestList.value = [] }
  },
  { immediate: true, deep: true }
)

function scheduleText(v){ const m={early:'早睡早起',normal:'正常作息',late:'晚睡晚起'}; return m[String(v)]||v||'—' }
function drinkingText(v){ const m={never:'从不',occasionally:'偶尔',often:'经常'}; return m[String(v)]||v||'—' }
function smokingText(v){ const m={never:'从不',occasionally:'偶尔',often:'经常'}; return m[String(v)]||v||'—' }
function workoutText(v){ const m={none:'无',weekly:'每周','3+weekly':'3次以上/周',daily:'每天'}; return m[String(v)]||v||'—' }
function petText(v){ const m={love:'喜爱',ok:'都OK',allergic:'过敏',reject:'拒绝'}; return m[String(v)]||v||'—' }
function spendingText(v){ const m={frugal:'节制理性',balanced:'均衡适度',luxury:'享受当下'}; return m[String(v)]||v||'—' }
function savingText(v){ const m={aggressive:'积极储蓄',balanced:'收入持平',conservative:'适度储蓄'}; return m[String(v)]||v||'—' }
function listOrDash(v){ if(!v && v!==0) return '—'; try{ if(Array.isArray(v)) return v.join('、'); const may=JSON.parse(v); if(Array.isArray(may)) return may.join('、') }catch{} return String(v) }
