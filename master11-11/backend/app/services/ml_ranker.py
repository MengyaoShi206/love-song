...
class HybridXgbDnnRanker:
    ...
    def __init__(self):
        ...
        self.dnn: Optional[Pipeline] = None
        self.selected_feats: List[str] = []
        self.expected_dim: Optional[int] = None
        # calib: ("platt", a, b) / ("iso", ISO_PATH, 0.0) / ("pct", p5, p95)
        self.calib: Optional[Tuple[str, float, float]] = None
        self.iso: Optional[IsotonicRegression] = None  # 仅在 iso 模式使用
