...
<template>
  <el-card class="box-card" style="max-width: 520px; margin: 24px auto;">
    <h2>登录</h2>
    <el-form :model="form" label-width="120px" @keyup.enter="submit">
      <el-form-item label="用户名">
        <el-input v-model="form.username" placeholder="请输入用户名" />
      </el-form-item>

      <el-form-item label="密码">
        <!-- show-password 自动添加眼睛图标 -->
        <el-input
          v-model="form.password_hash"
          placeholder="请输入密码或哈希"
          type="password"
          show-password
        />
      </el-form-item>
...
async function submit() {
  ...
  try {
    ...
    // ✅ 只在当前标签页记录登录用户，避免被其他标签页/后登录的账号覆盖
    sessionStorage.setItem('uid', String(data.id))
